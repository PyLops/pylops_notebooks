r"""
Test MDD with synthetic seismic data with cupy arrays

Uses https://github.com/DIG-Kaust/conda_envs/blob/main/environment_pylops_cupy_mpi4py_3090.yml

Run as: export OMP_NUM_THREADS=4; export MKL_NUM_THREADS=4; export NUMBA_NUM_THREADS=4; mpiexec -n 2 python mdd_cupy.py
"""
...
def run():
    ...
    # Define operator
    Fop = MPIMDC(cp.asarray(G), nt=2 * par["nt"] - 1, nv=1, nfreq=nf,
    ...
    x[:] = cp.asarray(m.astype(dtype).ravel())
    xloc = cp.asnumpy(x.asarray())

    y = Fop @ x
    yloc = cp.asnumpy(y.asarray().real)
    ...
    # Apply adjoint
    xadj = Fop.H @ y
    xadjloc = cp.asnumpy(xadj.asarray().real)
    ...
    xinvloc = cp.asnumpy(xinv.asarray().real)
