...
class MPIFredholm12Ddist(MPILinearOperator):
    ...
    of both the forward and adjoint are distributed arrays with scattered partition
    over the additional dimension: each rank takes a portion of such arrays over the first dimension,
    computes a partial integral, and the resulting outputs are then gathered by all ranks to return
    another distributed arrays with scattered partition over the additional dimension.
    ...
    def __init__(
        ...
        # Store subcommunicators and associated ranks
        ...
        self.mask = np.array(base_comm.allgather(self.rank_sl))

        # Store local and global dimensions of kernel
        self.nsl, self.nx, self.ny = G.shape
        self.nz = nz
        self.nsls = self.sub_comm_z.allgather(self.nsl)
        self.nslstot = np.sum(self.nsls)
    ...
    def _matvec(self, x: DistributedArray) -> DistributedArray:
        ...
        y = DistributedArray(global_shape=self.shape[0],
                             ...
                             partition=Partition.SCATTER, mask=self.mask,
        ...
        if self.usematmul:
            ...
            y1 = ncp.squeeze(ncp.zeros((self.nsls[self.rank_z], self.nx, self.nz_rank), dtype=self.dtype))
            for isl in range(self.nsls[self.rank_z]):
    ...
    def _rmatvec(self, x: NDArray) -> NDArray:
        ...
        if self.usematmul:
            ...
            y1 = ncp.squeeze(ncp.zeros((self.nsls[self.rank_z], self.ny, self.nz_rank), dtype=self.dtype))
            if hasattr(self, "GT"):
                for isl in range(self.nsls[self.rank_z]):
